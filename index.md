...
unless you’ve set up error tracking.
[glitchtipr](https://hyperverse-r.github.io/glitchtipr/) sends errors to
...
[glitchtipr](https://hyperverse-r.github.io/glitchtipr/) ships with a
`@capture` plumber2 tag. One annotation above a route — that’s all it
takes.
...
![GlitchTip issue list showing two captured R
errors](reference/figures/error.png)
...
![GlitchTip error detail showing endpoint, query string bins=a, and
request headers](reference/figures/error-context.png)
...
![GlitchTip uptime monitor showing the htmxr hello app available for 22
hours with ~50ms response times](reference/figures/monitor.png)
...
If no DSN is configured,
[`gt_connect()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_connect.md)
returns an inactive connection and
[`gt_capture()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_capture.md)
is a zero-overhead no-op. Safe in development without a GlitchTip
instance.

## Works with Shiny too

[`gt_capture()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_capture.md)
is framework-agnostic. In Shiny, use it directly inside `renderPlot()`,
`observeEvent()`, or any reactive context.
...
- **Errors propagate** —
  [`gt_capture()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_capture.md)
  uses
  [`withCallingHandlers()`](https://rdrr.io/r/base/conditions.html), not
  [`tryCatch()`](https://rdrr.io/r/base/conditions.html). Errors are
  reported but always re-raised. In plumber2 and Shiny render functions
  this is handled automatically. In Shiny observers, wrap with
  [`tryCatch()`](https://rdrr.io/r/base/conditions.html) after
  [`gt_capture()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_capture.md)
  to prevent the session from crashing.

- **Silent when unconfigured** — no DSN means no connection, which means
  [`gt_capture()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_capture.md)
  is a pure no-op. No errors, no overhead in development.

- **Minimal surface** — two functions:
  [`gt_connect()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_connect.md)
  and
  [`gt_capture()`](https://hyperverse-r.github.io/glitchtipr/reference/gt_capture.md).
  One plumber2 tag: `@capture`. That’s the entire API.

- **Self-hostable** — built for [GlitchTip](https://glitchtip.com) (open
  source, Sentry-compatible) but works with any platform that implements
  the Sentry store API.
